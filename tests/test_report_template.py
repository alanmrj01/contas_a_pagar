...
def test_month_filter_is_local_to_monthly_component():
    assert 'timelineMonthFilterOptions' in TEMPLATE
    assert 'state.timelineMonths' in TEMPLATE
    pass_fn = TEMPLATE.split('function pass(x)', 1)[1].split('function group', 1)[0]
    assert 'state.timelineMonths' not in pass_fn
...
def test_monthly_area_fits_up_to_three_blocks_and_has_stronger_hover():
    assert 'class="card cardWide monthlyMainCard"' in TEMPLATE
    assert 'class="monthlyMainLayout"' in TEMPLATE
    assert 'id="monthlyLocalKpis"' in TEMPLATE
    assert 'function linearTrend(values)' in TEMPLATE
    assert '.slice(0,3)' not in TEMPLATE
...
def test_filtered_pdf_uses_current_dom_and_print_has_no_scroll_areas():
    ...
    assert '.tableWrap,.warningDetails{max-height:none!important;height:auto!important;overflow:visible!important' in TEMPLATE
...
def test_lists_are_complete_in_html_and_static_pdf_is_paginated():
    ...
    assert 'Previsto x Realizado por mês' in PDF_SOURCE
    ...
    assert 'Detalhamento completo por fornecedor' in PDF_SOURCE
    assert 'Subcategoria' in PDF_SOURCE
...
def test_monthly_viewport_is_bounded_when_unfiltered():
    assert 'w=Math.max(820,150+months.length*135)' in TEMPLATE
    assert '.monthlyComparisonChart{height:390px;overflow-x:auto;overflow-y:hidden}' in TEMPLATE
    assert "style=\"min-width:'+w+'px\"" in TEMPLATE


def test_print_contrast_and_overflow_guards_exist():
    assert '.axisLab,.xLab,.smallLab,.valueLab{fill:#173047!important}' in TEMPLATE
    assert '.monthlyLocalKpi{padding:2mm!important;background:#fff!important' in TEMPLATE
...
def test_month_filter_options_follow_current_global_filters():
    assert 'function refreshTimelineMonthFilterOptions(p,r)' in TEMPLATE
    assert 'comparisonMonthsFromRows(p,r)' in TEMPLATE
    assert 'state.timelineMonths=state.timelineMonths.filter(month=>allowed.has(month))' in TEMPLATE
    timeline_render = TEMPLATE.split('function renderTimelineLocal()', 1)[1].split('\n', 1)[0]
    assert 'currentFilteredRows()' in timeline_render
...
def test_supplier_lollipop_names_and_values_are_larger_without_text_scaling():
    assert 'function lollipop(rows)' not in TEMPLATE
    assert 'def _lollipop(' not in PDF_SOURCE
    assert 'Detalhamento por fornecedor' in TEMPLATE
    assert 'Tabela completa conforme filtros atuais' in TEMPLATE
...
def test_supplier_period_context_and_waterfall_high_contrast_labels_are_present():
    assert 'id="waterfallMonthChip"' in TEMPLATE
    assert 'function categoryWaterfallData(p,r)' in TEMPLATE
...
def test_warning_classification_rows_have_accessible_selection_and_manual_apply_feedback():
    assert "'<tr class=\"classificationRow'+(usable?" in TEMPLATE
    ...
    assert 'data-correction-value=' in TEMPLATE
    assert 'Valor original' in TEMPLATE and 'Valor corrigido' in TEMPLATE
    assert TEMPLATE.count('data-submit-updates data-report-update-action>Atualizar relatório</button>') == 2
    assert 'Aplicar valores às selecionadas' not in TEMPLATE
    assert 'function submitWarningUpdates()' in TEMPLATE
    assert 'assignments,corrections,revision:reportBaseRevision' in TEMPLATE
...
def test_report_recalculation_has_nonblocking_loading_and_concurrency_guard():
    ...
    assert 'data-submit-updates data-report-update-action' in TEMPLATE
...
def test_report_initial_screen_scale_is_layout_based_responsive_and_print_safe():
    ...
    assert "Math.ceil(content.getBoundingClientRect().height)+'px'" in TEMPLATE
    assert ".style.zoom" not in TEMPLATE
...
def test_monthly_cards_require_a_filter_and_never_use_generic_latest_month_tag():
    assert 'monthlyCards' not in TEMPLATE
    assert 'id="monthlyLocalKpis"' in TEMPLATE
    for label in ('Previsto total', 'Realizado total', 'Desvio total', 'Variação total'):
        assert label in TEMPLATE


def test_every_graph_has_dynamic_global_and_local_filter_subtitle():
    for element_id in ('categoryFilterSummary', 'monthlyCardsFilterSummary', 'supplierFilterSummary', 'timelineFilterSummary'):
...
def test_monthly_cards_have_isolated_series_filter():
    assert 'id="timelineSeriesFilter"' in TEMPLATE
    assert "state.timelineSeries!=='actual'" in TEMPLATE
    assert "state.timelineSeries!=='planned'" in TEMPLATE
    pass_fn = TEMPLATE.split('function pass(x)', 1)[1].split('function group', 1)[0]
    assert 'timelineSeries' not in pass_fn
    assert "renderTimelineLocal()" in TEMPLATE
